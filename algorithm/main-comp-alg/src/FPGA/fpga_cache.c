...
NiFpga_Status refresh_cache(Fpga *fpga) {
	...
	if(NiFpga_IsNotError(fpga->status)){
		if(NiFpga_MergeStatus(&(fpga->status),  NiFpga_ReadBool(fpga->session, 0x1805A, &tempNiFpga_Bool))){
	...
	if(NiFpga_IsNotError(fpga->status)){
		if(NiFpga_MergeStatus(&(fpga->status),  NiFpga_ReadBool(fpga->session, 0x18166, &tempNiFpga_Bool))){
			STORE((fpga->cache.frg_run_state), tempNiFpga_Bool);
		}
	}
	/* Atomically store fuse_avb_p1 */
	if(NiFpga_IsNotError(fpga->status)){
		if(NiFpga_MergeStatus(&(fpga->status),  NiFpga_ReadBool(fpga->session, 0x18106, &tempNiFpga_Bool))){
			STORE((fpga->cache.fuse_avb_p1), tempNiFpga_Bool);
		}
	}
	/* Atomically store fuse_avb_p2 */
	if(NiFpga_IsNotError(fpga->status)){
		if(NiFpga_MergeStatus(&(fpga->status),  NiFpga_ReadBool(fpga->session, 0x1810A, &tempNiFpga_Bool))){
	...
	if(NiFpga_IsNotError(fpga->status)){
		if(NiFpga_MergeStatus(&(fpga->status),  NiFpga_ReadU8(fpga->session, 0x18102, &tempuint8_t))){
			STORE((fpga->cache.cur_thresh_index), tempuint8_t);
		}
	}
	/* Atomically store light_BL_count */
	if(NiFpga_IsNotError(fpga->status)){
		if(NiFpga_MergeStatus(&(fpga->status),  NiFpga_ReadU8(fpga->session, 0x180AA, &tempuint8_t))){
	...
	if(NiFpga_IsNotError(fpga->status)){
		if(NiFpga_MergeStatus(&(fpga->status),  NiFpga_ReadU8(fpga->session, 0x1814A, &tempuint8_t))){
	...
	if(NiFpga_IsNotError(fpga->status)){
		if(NiFpga_MergeStatus(&(fpga->status),  NiFpga_ReadI16(fpga->session, 0x1816A, &tempint16_t))){
			STORE((fpga->cache.T_cRIO), tempint16_t);
		}
	}
	/* Atomically store P_hp1 */
	if(NiFpga_IsNotError(fpga->status)){
		if(NiFpga_MergeStatus(&(fpga->status),  NiFpga_ReadI32(fpga->session, 0x180BC, &tempfxp32_16))){
	...
	if(NiFpga_IsNotError(fpga->status)){
		if(NiFpga_MergeStatus(&(fpga->status),  NiFpga_ReadI32(fpga->session, 0x180EC, &tempfxp32_16))){
	...
	if(NiFpga_IsNotError(fpga->status)){
		if(NiFpga_MergeStatus(&(fpga->status),  NiFpga_ReadI32(fpga->session, 0x180E4, &tempfxp32_16))){
			STORE((fpga->cache.mc_distance), tempfxp32_16);
		}
	}
	/* Atomically store tape_velocity */
	if(NiFpga_IsNotError(fpga->status)){
		if(NiFpga_MergeStatus(&(fpga->status),  NiFpga_ReadI32(fpga->session, 0x180B8, &tempfxp32_16))){
			STORE((fpga->cache.tape_velocity), tempfxp32_16);
		}
	}
	/* Atomically store voltage_p1 */
	if(NiFpga_IsNotError(fpga->status)){
		if(NiFpga_MergeStatus(&(fpga->status),  NiFpga_ReadI32(fpga->session, 0x1804C, &tempfxp32_16))){
	...
	if(NiFpga_IsNotError(fpga->status)){
		if(NiFpga_MergeStatus(&(fpga->status),  NiFpga_ReadI32(fpga->session, 0x180CC, &tempint32_t))){
			STORE((fpga->cache.tape_distance), tempint32_t);
		}
	}
	/* Atomically store cur_thresh_persistence */
	if(NiFpga_IsNotError(fpga->status)){
		if(NiFpga_MergeStatus(&(fpga->status),  NiFpga_ReadU32(fpga->session, 0x180FC, &tempuint32_t))){
			STORE((fpga->cache.cur_thresh_persistence), tempuint32_t);
		}
	}
	/* Atomically store cur_thresh_ratio */
	if(NiFpga_IsNotError(fpga->status)){
		if(NiFpga_MergeStatus(&(fpga->status),  NiFpga_ReadU32(fpga->session, 0x180F8, &tempuint32_t))){
			STORE((fpga->cache.cur_thresh_ratio), tempuint32_t);
		}
	}
	/* Atomically store imu_distance */
	if(NiFpga_IsNotError(fpga->status)){
		if(NiFpga_MergeStatus(&(fpga->status),  NiFpga_ReadI64(fpga->session, 0x1814C, &tempfxp64_32))){
	...
	if(NiFpga_IsNotError(fpga->status)){
		if(NiFpga_MergeStatus(&(fpga->status),  NiFpga_ReadU64(fpga->session, 0x1803C, &tempuint64_t))){
...
NiFpga_Status write_enable_checks(Fpga *fpga, NiFpga_Bool v) {
	NiFpga_IfIsNotError(fpga->status, NiFpga_WriteBool(fpga->session, 0x180D6, v));
	return fpga->status;
}

NiFpga_Status write_frg_run(Fpga *fpga, NiFpga_Bool v) {
	NiFpga_IfIsNotError(fpga->status, NiFpga_WriteBool(fpga->session, 0x18162, v));
	return fpga->status;
}
...
NiFpga_Status write_iso_led_1(Fpga *fpga, NiFpga_Bool v) {
	NiFpga_IfIsNotError(fpga->status, NiFpga_WriteBool(fpga->session, 0x18086, v));
	return fpga->status;
}
...
NiFpga_Status write_reset_tape_count(Fpga *fpga, NiFpga_Bool v) {
	NiFpga_IfIsNotError(fpga->status, NiFpga_WriteBool(fpga->session, 0x1815E, v));
	return fpga->status;
}
...
NiFpga_Status write_imu_acc(Fpga *fpga, fxp32_16 v) {
	NiFpga_IfIsNotError(fpga->status, NiFpga_WriteI32(fpga->session, 0x18154, v));
	return fpga->status;
}

NiFpga_Status write_mc_velocity(Fpga *fpga, fxp32_16 v) {
	NiFpga_IfIsNotError(fpga->status, NiFpga_WriteI32(fpga->session, 0x180E8, v));
	return fpga->status;
}
...
NiFpga_Status write_imu_timestamp(Fpga *fpga, fxp64_32 v) {
	NiFpga_IfIsNotError(fpga->status, NiFpga_WriteI64(fpga->session, 0x18158, v));
	return fpga->status;
}
